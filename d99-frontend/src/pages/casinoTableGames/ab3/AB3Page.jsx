...
import BetTableAB3 from "../../../components/casino/tables/ab3/BetTableAB3.jsx";
import AB3VideoCards from "../../../components/casino/tables/ab3/AB3VideoCards.jsx";
import {
...
const GAME_ID = "ab3";
const GAME_NAME = "ANDAR BAHAR 50 CARDS";

function formatAB3Result(r) {
  return {
    label: "R",
...
export default function AB3Page() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setLastResults((d.res || []).slice(0, 10).map(formatAB3Result));
      } catch { /* ignore */ }
    };
    fetch();
  ...
  // Parse next card count and side from sub or raw data
  const nextCardCount = raw?.nc || raw?.nextCard || 0;
  const nextCardSide = raw?.ns || raw?.nextSide || "";
        ...
        <div className={mobilePanelTab === "bets" ? "d-none d-xl-block" : ""}>
          ...
          {sub.length > 0 && (
            <div className="casino-detail">
              <BetTableAB3 gameData={gameData} onBetClick={handleBetClick} exposures={exposures} />
