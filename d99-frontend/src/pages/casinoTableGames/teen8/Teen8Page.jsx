...
import BetTableTeen8 from "../../../components/casino/tables/teen8/BetTableTeen8.jsx";
...
function formatResult(r) {
  ...
  // teen8 results show "R" with result-b, or player number
  return {
    label: r.result || "R",
...
export default function Teen8Page() {
  const {
  ...
  return (
    <Layout variant="casino-page"
      ...
      <div className="casino-page-container teenpattiopen">
        {loading && <CasinoLoader />}
        <CasinoHeader name="Teenpatti Open" gameId={GAME_ID} roundId={roundId} />
        ...
        <div className={mobilePanelTab === "bets" ? "d-none d-xl-block" : ""}>
          <CasinoVideoBox src={iframeSrc} gameName="Teenpatti Open" gameType="teen8" cardString={cardString} clock={<FlipClock value={timer} />} />
          {tableData.length > 0 && <div className="casino-detail">
            <BetTableTeen8 tableData={tableData} onBetClick={handleBetClick} exposures={exposures} cardString={cardString} />
      ...
      <CasinoResultModal show={resultModal.show} onHide={() => setResultModal({ show: false, mid: "" })} gameId={GAME_ID} gameType="teen8" mid={resultModal.mid} title="Teenpatti Open Result" />
    </Layout>
  );
}
