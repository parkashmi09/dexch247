...
export default function Card32EUResultContent({ detailData }) {
  ...
  // rdesc: "Player 11#8 : Even  |  9 : Even~10 : Odd  |  11 : Odd#Black#10-11#6"
  const descParts = rdesc ? rdesc.split("#") : [];
  const winner = descParts[0] || winnat || "";
  const oddEvenRaw = descParts[1] || "";
  const oddEvenLines = oddEvenRaw.split("~");
  const blackRed = descParts[2] || "";
  const total = descParts[3] || "";
  const single = descParts[4] || "";
        ...
        <div className="col-md-6">
          <div className="casino-result-desc">
            ...
            {oddEvenLines[0] && (
              <div className="casino-result-desc-item">
                <div>Odd/Even</div>
                <div>{oddEvenLines[0]}</div>
              </div>
            )}
            {oddEvenLines[1] && (
              <div className="casino-result-desc-item">
                <div>&nbsp;</div>
                <div>{oddEvenLines[1]}</div>
              </div>
            )}
            {blackRed && (
              <div className="casino-result-desc-item">
