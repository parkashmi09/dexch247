...
export default function JokerVideoCards({ cardString = "" }) {
  const tokens = cardString.split(",").map((t) => t.trim());

  // card string: token[0]=joker, then player cards are dealt alternately —
  // [1]=pA card1, [2]=pB card1, [3]=pA card2, [4]=pB card2, [5]=pA card3, [6]=pB card3
  const jokerCard = getCardImage(tokens[0]);
