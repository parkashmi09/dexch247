...
function MarketRow({ item, onBetClick, exposures }) {
  ...
  const exp = exposures?.[item.nat] ?? exposures?.[item.nat?.toLowerCase()] ?? null;
  const expN = exp !== null ? parseFloat(exp) : NaN;

  // Blink on odds change
        ...
        <div className="market-row">
          ...
          <div className="fancy-min-max-box">
            <div className="fancy-min-max">
              <span className="w-100 d-block">Min: {formatAmount(item.min || 100)}</span>
              <span className="w-100 d-block">Max: {formatAmount(item.max || 100000)}</span>
            </div>
          </div>
        </div>
      </div>
...
function MarketSection({ title, items, onBetClick, exposures }) {
  return (
...
export default function GoalMarkets({ tableData = [], onBetClick, exposures = {} }) {
  const players = tableData.filter((d) => d.subtype === "player");
  const methods = tableData.filter((d) => d.subtype === "goal");

  return (
    <>
      {players.length > 0 && (
        <MarketSection title="Who Will Goal Next?" items={players} onBetClick={onBetClick} exposures={exposures} />
      )}
      {methods.length > 0 && (
        <MarketSection title="Method Of Next Goal" items={methods} onBetClick={onBetClick} exposures={exposures} />
      )}
    </>
  );
}
