...
export default function LottcardResultContent({ detailData }) {
  const t1 = detailData?.data?.data?.t1 || detailData?.data?.t1 || {};
  const { card = "", mtime = "", rdesc = "", rid = "", win = "", winnat = "" } = t1;

  const cards = card.split(",").map((c) => c.trim()).filter(Boolean);
  const resultText = rdesc || winnat || win || "";
        ...
        <div className="col-md-12 text-center">
          <div className="casino-result-cards">
            {cards.map((code, i) => {
              ...
              return src ? <img key={i} src={src} alt={code} /> : null;
            })}
          </div>
        </div>
      </div>
