...
const ResultTeen32 = ({ 
  ...
  useEffect(() => {
    const fetchLastResults = async () => {
      try {
        const response = await getLastResults('teen32');
  ...
  const handleResultClick = async (result, index) => {
    ...
    if (mid) {
      try {
        const response = await getDetailResults('teen32', mid.toString());
  ...
  const getModalData = () => {
    ...
    // rdesc is just "Player A" or "Player B" (simple format, no #)
    const winner = detailResult?.winnat || detailResult?.rdesc || (selectedResult === "A" ? "Player A" : selectedResult === "B" ? "Player B" : "");
    
    // Determine winner from API data - check win field first, then winnat/rdesc, then selectedResult
    ...
    <>
      ...
      {modalData && (
        <ResultModalLayout
          ...
          title={detailResult?.ename || "Instant Teenpatti 2.0 Result"}
        ...
        >
          ...
          {/* Result Details - Only ONE box for Winner */}
