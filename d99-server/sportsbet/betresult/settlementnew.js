...
// ==== API Layer ====
async function fetchResultForEvent(eventid, eventName, marketId, marketName, sport_id, market_type, gameType) {
    log.debug('[Settlement] fetchResultForEvent start', { eventid, eventName, marketId, marketName });
...
// api layer for events but fancy 
async function fetchResultForEventFancy(eventid, eventName, marketId, selection_name, sport_id, market_type) {
    log.debug('[Settlement] fetchResultForEvent start', { eventid, eventName, marketId, selection_name });
    const maxAttempts = RESULTS_MAX_FETCH_RETRIES || 6;
    const url = `${RESULTS_API_BASE}/result/get-result`;
...
function resolveFancyWinner({ result, fancy_name, selection_name, team_one, team_two, bet_type, market_type, odds }) {
    ...
    let final_result = result.winnerId;
    ...
    if (market_type === 'oddeven') {
        ...
        // special condition
        final_result = result.winnerName;
        if (final_result === 'Yes' || final_result === 'yes' || final_result === 'YES' && bet_type === 'yes' || bet_type === 'YES' || bet_type === 'Yes') {
            winnerName = 'won';
        }
        else if (final_result === 'No' || final_result === 'no' || final_result === 'NO' && bet_type === 'no' || bet_type === 'NO' || bet_type === 'No') {
            winnerName = 'won';
        }
        else {
            winnerName = 'loss';
...
async function initiateRefund({ job_id, user_id, eventid, match_id, bet, market_type }) {
    ...
    try {
        ...
        // === EXPOSURE-AWARE REFUND (see settlementv2.js for full rationale) ===
        // Reverse the wallet delta caused at placement time, instead of
        // unconditionally crediting `stake` (which double-pays hedging bets).
...
async function processMobmGroup({ job_id, user_id, eventid, match_id, bets, eventName, marketId, marketName, market_type, game_type, team_one, team_two, sport_id }) {
    ...
    if (any.status === 'manual') {
        ...
        result = await fetchResultForEvent(eventid, eventName, marketId, marketName, sport_id, market_type);
    }
    ...
    // e.g. blocked=340, winner=-100 → 340+(-100)=240 (partial return)
    // e.g. blocked=200, winner=+300 → 200+300=500 (return + profit)
    let marketFinalCredit = 0;
    if (winnerName === 'refund') {
        marketFinalCredit = normalizeExposure(mMostNeg); // return full blocked amount
    } else {
