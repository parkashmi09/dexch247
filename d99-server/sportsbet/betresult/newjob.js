...
const RESULTS_API_BASE = 'https://apilords.codefactory.games/api';
...
async function runOnce() {
    ...
    try {
        ...
        const allBets = await SportsBet.findAll({
            ...
            attributes: ['id', 'user_id', 'game_type', 'bet_type', 'selection_name', 'odds', 'stake_amount', 'match_title', 'match_id', 'fancy_name', 'eventid', 'created_at', 'sport_id', 'market_type', 'status'],
            order: [['created_at', 'ASC']]
        });
        ...
        for (const b of allBets) {
            try {
                ...
                if (b.status === 'manual') {
                    ...
                    const res = await fetchResultForEvent(eid, eventName, marketId, marketName, sport_id, market_type, gameType);
